...
<%-- page 지시문 trimDirectiveWhitespaces 속성:
    jsp파일이 java 코드로 변환되는 과정에서 JSP 태그들이 빈줄로 대체 되는데,
    빈 줄을 삭제(true)할 것인 지 아닌 지(false)를 설정하는 속성.
 --%>
...
2. JSP(Java/Jakarta Server Page)
    ...
    (3) JSP 동작 원리:
      o. 최초 요청 -> WAS가 jsp 파일을 java 파일로 변환 -> java 코드를 class로 컴파일
        -> 객체 생성 -> 메서드 호출 -> 응답      
...
<%!
/* JSP declaration */
private static final String USER = "scott"; // 상수 필드
