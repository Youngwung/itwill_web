...
public class ForwardServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 요청을 처리할 수 있는 서블릿 클래스 객체의 메서드(doGet 또는 doPost)를 호출.
		// 서블릿에서는 HTML 코드를 작성해서 응답을 보내면 됨.
		// 서블릿에서 HTML을 작성하는 것은 너무 번거로움.
		// 서블릿이 JSP로 요청을 전달하고, JSP가 HTML 코드를 작성하는 것이 더 쉬움.
		
		request.getRequestDispatcher("example.jsp").forward(request, response);
		// "example.jsp"로 이동(forward)하는 메서드.
		
		// "forward" 방식의 웹 페이지 이동:
		// - 같은 WAS의 같은 웹 애플리케이션 안에서 페이지를 이동하는 방식.
		// - 최초 요청 주소가 바뀌지 않음.(서블릿 url패턴 ex3에서 바뀌지 않음)
