...
public class CommentRestController {
  ...
  public ResponseEntity<List<CommentItemDto>> getAllCommentByPostId(@PathVariable(name = "postId") int postId) {
    ...
    // jackson-datatype-jsr310 모듈(애드온)이 필요함.
  }

  @GetMapping("/{id}") // -> /api/comment/{id}
  // 댓글번호로 댓글 검색
  public ResponseEntity<CommentItemDto> getReplyById(@PathVariable(name = "id") int id) {
  ...
  @PostMapping // 요청 주소가 /api/comment일 때 매핑 주소를 쓰지 않는다.
  // 댓글 생성
