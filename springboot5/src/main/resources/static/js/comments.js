...
document.addEventListener('DOMContentLoaded', () => {
		...
		axios.get(uri)
			.then((response) => {
				...
				if (currentPageNo + 1 < totalPages) {
					// (!response.data.last): 현재 페이지가 마지막 페이지 인지(T/F)
	...
	function makeCommentElements(data, pageNo) {
		...
		const authUser = document.querySelector('#authenticatedUser').innerText;
		...
		function updateComment(event) {
			...
			// 클래스 이름이 commentText이고, data-id 속성이 id인 요소를 찾음.
