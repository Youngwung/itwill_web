...
// 배열 선언 & 초기화
const arr = [1, 2, 30, 50, 80]; // 자바: int[] arr = {1, 2, ...};
...
// for-of 문장: 배열의 원소들을 interation(순회).
html = '';
for (let val of arr) {
    html += `${val}, `
}
 ...
 html = `배열 arr의 합계는 ${sum}, 평균은 ${avg} 입니다.`;
 
 output.innerHTML += html + '</br>';
 

 // 연습문제: 배열의 원소들을 ul의 li 요소로 만들기: 
 ...
 for (let idx in drama) {
    html += `<li>${Number(idx)+1} : ${drama[idx]}</li>`;
 }
 list.innerHTML += html;
