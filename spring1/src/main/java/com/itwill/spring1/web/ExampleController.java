...
// POKO(Plain Old C++(C#) Object)
// POJO(Plain Old Java Object): 간단한 오래된 자바 객?체
// 특정 클래스를 상속(extends)하거나, 특정 인터페이스를 구현(impolements)할 필요가 없는 
// (상위 타입의 특정 메서드들을 반드시 재정의할 필요가 없는) 평범한 자바 객체.
// 스프링 MVC 프레임워크에서는 POJO로 작성된 클래스를 컨트롤러로 사용할 수 있다.(애너테이션으로)
...
public class ExampleController {
  ...
  public String home(Model model) {
    ...
    return "home";
    // -> 컨트롤러 메서드가 문자열을 리턴하면, 디스패쳐 서블리싱 뷰의 이름을 찾는 데 사용.
    // 디스패쳐 서블릿이 뷰 리졸버를 이용해서 /WEB-INF/views/"returnValue".jsp" 경로를 찾을 수 있다.
    // 이 경로는 servlet-context.xml 파일의 <bean>태그 안의 prefix, suffix 속성으로 설정할 수 있다.
  }

  @GetMapping("/example")
  public void controllerExmple() {
    log.debug("controllerExmple()");
    // 컨트롤러 메서드가 리턴 값이 없는 경우,
    // 요청 주소가 뷰의 이름이 됨. 이 메서드는 views안에 있는 example.jsp를 찾음.
    // 요청주소와 파일 이름이 똑같으면 굳이 리턴할 필요 없음.
  }
  ...
  public String ex2(@ModelAttribute(name = "user") UserDto dto) {
    ...
    // 생성된객체를 컨트롤러 메서드의 아규먼트로 전달.

    // -> @ModelAttribute(name = "user") UserDto dto 파라미터 선언은
    // model.addAttribute("user", dto); 코드 작성과 같은 효과.
    // 컨트롤러에서 뷰로 전달하는 데이터
    // *아규먼트로 모델을 받지 않고 model.addAtbt도 안쓸 수 있음
    // *아규먼트로 @ModelAttribute(name = "user") UserDto dto를 주면 가능.
    // *코드를 많이 간소화할 수 있음.
    return "ex1"; // -> 뷰 이름
  }
  ...
  public String forward() {
    ...
    // 포워드방식의 페이지 이동 - 최초 요청 주소가 바뀌지 않음! => req 파라미터가 유지됨.
  }
  ...
  public String redirect() {
    ...
    // 리다이렉트 방식의 페이지 이동 - 최초의 요청 주소가 리다이렉트 되는 주소로 바뀜.
  }
  ...
  @GetMapping("/rest2")
  @ResponseBody // -> 리턴 값이 클라이언트로 지접 응답되는 객체.
  public UserDto rest2() {
    ...
    // JASON(JavaScript Object Notation) 형식의 문자열로 변환하고,
    // 클라이언트로 응답을 보냄.
  }
